...
fpara = [0.2 0.5 0.3];
infolder = 'E:\code\pytorch-salgan360\input\';
outfolder = 'E:\code\pytorch-salgan360\output\';
...
for v = 1:length(headmove_v)  
    ...
    for i=1:6
        filename = saliencyList(i+(v-1)*6*length(headmove_h)+(h-1)*6+2).name;
%         cubsal = double(imresize(imread([outfolder 'fov120_pred_vp\' filename]), [1026, 1026]));
        cubsal = double(imread([outfolder 'fov120_pred_vp\' filename]));
        fov120(:,:,1,i) = cubsal;
        fov120(:,:,2,i) = cubsal;
        fov120(:,:,3,i) = cubsal;
